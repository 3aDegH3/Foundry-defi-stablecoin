// SPDX-License-Identifier: SEE LICENSE IN LICENSE
pragma solidity ^0.8.18;

// Imports
import {MockV3Aggregator} from "../test/mocks/MockV3Aggregator.sol";
import {Script} from "forge-std/Script.sol";
import {ERC20Mock} from "../test/mocks/ERC20Mock.sol";

/**
 * @title HelperConfig
 * @notice Configuration helper for deploying the DSC system across different networks
 * @dev Provides network-specific configurations and deploys mocks for local development
 */
contract HelperConfig is Script {
    /*/////////////////////////////////////////////////////////////
                                TYPES
    /////////////////////////////////////////////////////////////*/

    struct NetworkConfig {
        address wethUsdPriceFeed;
        address wbtcUsdPriceFeed;
        address weth;
        address wbtc;
        uint256 deployerKey;
    }

    /*/////////////////////////////////////////////////////////////
                            CONSTANTS
    /////////////////////////////////////////////////////////////*/

    uint8 public constant DECIMALS = 8;
    uint256 public constant ETH_USD_PRICE = 2000e8;
    uint256 public constant BTC_USD_PRICE = 1000e8;
    uint256 public constant DEFAULT_ANVIL_PRIVATE_KEY =
        0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;

    /*/////////////////////////////////////////////////////////////
                            STATE VARIABLES
    /////////////////////////////////////////////////////////////*/

    NetworkConfig public activeNetworkConfig;

    /*/////////////////////////////////////////////////////////////
                              FUNCTIONS
    /////////////////////////////////////////////////////////////*/

    constructor() {
        // Initialize the appropriate network configuration
        if (block.chainid == 11155111) {
            // Sepolia
            activeNetworkConfig = getSepoliaEthConfig();
        } else {
            // Local or unsupported network
            activeNetworkConfig = getOrCreateAnvilEthConfig();
        }
    }

    /**
     * @notice Get the Sepolia network configuration
     * @return NetworkConfig The configuration for Sepolia network
     */
    function getSepoliaEthConfig() public view returns (NetworkConfig memory) {
        return
            NetworkConfig({
                wethUsdPriceFeed: 0x694AA1769357215DE4FAC081bf1f309aDC325306,
                wbtcUsdPriceFeed: 0x1b44F3514812d835EB1BDB0acB33d3fA3351Ee43,
                weth: 0xdd13E55209Fd76AfE204dBda4007C227904f0a81,
                wbtc: 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063, // TODO: Verify WBTC address
                deployerKey: vm.envUint("PRIVATE_KEY")
            });
    }

    /**
     * @notice Get or create Anvil (local) network configuration
     * @dev Deploys mock contracts if they don't exist
     * @return NetworkConfig The configuration for Anvil network
     */
    function getOrCreateAnvilEthConfig() public returns (NetworkConfig memory) {
        // Return existing config if already set
        if (activeNetworkConfig.wethUsdPriceFeed != address(0)) {
            return activeNetworkConfig;
        }

        // Deploy mock contracts
        vm.startBroadcast(DEFAULT_ANVIL_PRIVATE_KEY);
        MockV3Aggregator ethUsdPriceFeed = new MockV3Aggregator(
            DECIMALS,
            int256(ETH_USD_PRICE)
        );
        ERC20Mock wethMock = new ERC20Mock("WETH", "WETH", tx.origin, 1000e18);

        MockV3Aggregator btcUsdPriceFeed = new MockV3Aggregator(
            DECIMALS,
            int256(BTC_USD_PRICE)
        );
        ERC20Mock wbtcMock = new ERC20Mock("WBTC", "WBTC", tx.origin, 1000e8);
        vm.stopBroadcast();

        // Create and store new config
        NetworkConfig memory config = NetworkConfig({
            wethUsdPriceFeed: address(ethUsdPriceFeed),
            wbtcUsdPriceFeed: address(btcUsdPriceFeed),
            weth: address(wethMock),
            wbtc: address(wbtcMock),
            deployerKey: DEFAULT_ANVIL_PRIVATE_KEY
        });

        activeNetworkConfig = config;
        return config;
    }
}
